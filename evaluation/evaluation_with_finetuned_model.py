...
try:
    ...
    print("Warning: Geochat module not found or failed to import.")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="geochat",
        choices=["geochat", "qwen2_5_vl", "rs_llava", "internvl2_5", "georeason", "qwen3vl", "openai"],
        help="Model backend: geochat | qwen2_5_vl | rs_llava | internvl2_5 | georeason | qwen3vl | openai",
    )
    parser.add_argument(
        "--qwen2-vl-model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="gpt-4o",
        help="OpenAI 模型名称，如 gpt-4o、gpt-4-turbo 等（--backend=openai 时使用）",
    )
    parser.add_argument(
        "--openai-api-key",
        type=str,
        default=None,
        help="OpenAI API Key（也可通过 OPENAI_API_KEY 环境变量指定）",
    )
    parser.add_argument(
        "--openai-base-url",
        type=str,
        default=None,
        help="OpenAI 兼容 API 的 base URL（如使用第三方代理或本地部署时指定）",
    )
    parser.add_argument(
        "--openai-max-retries",
        type=int,
        default=3,
        help="OpenAI API 调用失败时的最大重试次数",
    )
    parser.add_argument(
        "--openai-retry-delay",
        type=float,
        default=5.0,
        help="OpenAI API 重试间隔秒数",
    )
    parser.add_argument(
        "--system-prompt",
        type=str,
        default=None,
        help="Global system prompt to inject for API or models that support it",
    )
    parser.add_argument(
        "--qids-file",
        type=Path,
...
def build_qwen2_5_vl(model_path: str, device: torch.device, lora_path: Path | None = None):
    ...
    # 尝试使用 flash_attention_2 加速（如果可用）
    # 注意：如果 flash-attn 未安装或不可用，会自动回退到 sdpa 或 eager
    # attn_implementation = None
    # try:
    #     import flash_attn
    #     attn_implementation = "flash_attention_2"
    #     print(f"Using flash_attention_2 for faster inference")
    # except ImportError:
    #     # flash-attn 未安装，使用默认的 sdpa（PyTorch 2.0+ 默认，比 eager 快）
    #     attn_implementation = "sdpa"
    #     print(f"flash-attn not available, using sdpa attention (faster than eager)")
...
def build_qwen3_vl(model_path: str, device: torch.device, lora_path: Path | None = None):
    ...
    if not os.path.exists(model_path):
        raise ValueError(f"Model path does not exist: {model_path}. Please ensure the path is correct and points to a local directory.")
...
def build_rs_llava(model_path: Path, lora_path: Path, device: torch.device):
    ...
    if "Merged" in model_path_str:
        ...
        if not os.path.exists(model_base):
             print(f"Warning: Base model path {model_base} does not exist. Trying HF Hub ID 'Intel/neural-chat-7b-v3-3'...")
...
def build_georeason(model_path: str, device: torch.device, lora_path: Path | None = None):
    ...
    if Qwen2_5_VLForConditionalGeneration is None:
        raise ImportError("Qwen2_5_VLForConditionalGeneration not available. Please install transformers with Qwen2.5-VL support.")
...
def collate_batch_rs_llava(batch, tokenizer, model, image_processor, image_dir: Path, conv_mode: str, device: torch.device):
    input_ids_list = []
    images_list = []
    question_ids = []
...
def build_openai(api_key: str | None = None, base_url: str | None = None):
    """构建 OpenAI 客户端"""
    if not HAS_OPENAI:
...
def generate_predictions_openai(
    ...
    """使用 OpenAI API 进行多模态生成（逐条调用，支持重试）"""
    ...
    progress = tqdm(indices, desc=f"Generating(OpenAI/{model_name})", disable=rank != 0)
    for idx in progress:
        sample = samples[idx]
        ...
        # 构造多模态 content：先图片后文本
        content_items: List[dict] = []
        for rel_path in img_list:
            abs_path = str((image_dir / rel_path).expanduser().resolve())
            b64_image = _encode_image_to_base64(abs_path)
            mime_type = _get_image_mime_type(abs_path)
        ...
        # 构造消息列表
        system_prompt = global_system_prompt or sample.get("system") or sample.get("system_prompt")
        messages: List[dict] = []
        if system_prompt:
            messages.append({"role": "system", "content": system_prompt})
        messages.append({"role": "user", "content": content_items})

        # 调用 OpenAI API（流式生成 + 指数退避重试）
        # timeout: connect=10s，两个 chunk 之间最长等待 60s
        import httpx
        _timeout = httpx.Timeout(connect=10.0, read=60.0, write=10.0, pool=10.0)
        # kimi 系列模型仅支持 temperature=1
        _temperature = 1 if "kimi" in model_name.lower() else temperature
        answer = ""
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print(f"OpenAI API error (final attempt): {e}. Returning empty answer.")
...
def main() -> None:
    ...
    if args.backend == "geochat":
        ...
        client = build_openai(args.openai_api_key, args.openai_base_url)
        local_records = generate_predictions_openai(
            samples=samples,
            indices=indices,
            client=client,
            ...
            retry_delay=args.openai_retry_delay,
...
# python -m torch.distributed.run --nproc_per_node=8 src/evaluation_with_finetuned_model.py \
# --base-model /mnt/hanhc/GeoChat/geochat-7B \
# --dataset /mnt/hanhc/TLM-main/data/the7/MCQ/NWPU_caption_negated_llava_format_shuffle.jsonl \
# --image-dir /mnt/hanhc/Remote_Data/MCQ/NWPU_Captions/RESISC45_IMG \
# --batch-size 16 \
# --temperature 0.0 \
# --lora-path /mnt/hanhc/TLM-main/saves/geochat-7b/vl_ttl/NWPU_5e-5_1gpu_shuffle_lambda_0_max20_no_system \
# --output outputs/NWPU_5e-5_1gpu_shuffle_lambda_0_max20_no_system.jsonl

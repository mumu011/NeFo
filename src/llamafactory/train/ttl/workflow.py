...
class TTLModel(nn.Module):
    ...
    def forward_for_offline(self, train_batch, predict_batch):  
        ...
        """
        print(f"🚀 Starting OFFLINE TTL Training...")
        print(f"📊 Training dataset size: {len(train_batch)} samples")
        print(f"📊 Evaluation dataset size: {len(predict_batch)} samples")
        ...
        print(f"✅ Prediction completed and results saved!")
    ...
    def forward_for_online(self, train_batch, predict_batch):
        ...
        self.reset_trainer(train_dataset=train_batch)
        print(f"🔄 Training on current batch with {len(train_batch)} samples...")
        self.trainer.train(resume_from_checkpoint=self.training_args.resume_from_checkpoint)
...
def run_ttl(
    ...
    print(f"   Total training steps: {training_args.max_steps or 'auto'}")
    ...
    if finetuning_args.setting == "offline_ttl":
        ...
        for k in range(num_of_batch):
            print(f"🔄 Processing streaming batch {k+1}/{num_of_batch} with size {streaming_batch_size}")
            ...
            print(f"   Current batch: {len(sub_trainset)} training samples, {len(sub_evalset)} eval samples")

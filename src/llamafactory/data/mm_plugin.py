...
if is_transformers_version_greater_than("4.52.0"):
    ...
    try:
        ...
        make_flat_list_of_images = None  # type: ignore
        make_batched_videos = None  # type: ignore
...
def _check_video_is_nested_images(video: "VideoInput") -> bool:
    ...
    return isinstance(video, list) and all(
        isinstance(frame, (str, BinaryIO, dict, ImageObject)) for frame in video
    )
    ...
    def _regularize_videos(self, videos: list["VideoInput"], **kwargs) -> dict[str, list[list["ImageObject"]]]:
        ...
        for video in videos:
            ...
            if _check_video_is_nested_images(video):
                for frame in video:
                    if not is_valid_image(frame) and not isinstance(frame, dict) and not os.path.exists(str(frame)):
...
class InternVLPlugin(BasePlugin):
    ...
    def _get_mm_inputs(
        ...
        **kwargs,
        ...
        if len(images) != 0 and make_flat_list_of_images is not None:
            ...
            # Fallback when make_flat_list_of_images not available: treat each image as one patch
            image_inputs = image_processor(images, return_tensors="pt", **image_processor_kwargs)
            _np = image_inputs.get("num_patches", None) if isinstance(image_inputs, dict) else getattr(image_inputs, "num_patches", None)
            if _np is None:
                image_num_patches = [1] * len(images)
            elif hasattr(_np, "tolist"):
                image_num_patches = _np.tolist()
            elif isinstance(_np, (list, np.ndarray)):
                image_num_patches = list(_np)
            else:
                image_num_patches = [1] * len(images)
            image_pixel_values = image_inputs.get("pixel_values", None) if isinstance(image_inputs, dict) else getattr(image_inputs, "pixel_values", None)
            if image_pixel_values is None and hasattr(image_inputs, "pop"):
                image_pixel_values = image_inputs.pop("pixel_values", None)
            image_num_patches_indices = np.cumsum(image_num_patches)

        if len(videos) != 0 and make_batched_videos is not None:
            ...
            video_num_patches = None
            video_num_patches_indices = None
            patch_indices = None

        # NOT SUPPORT IMAGE VIDEO INTERLEAVED
        if len(images) != 0 and image_pixel_values is not None:
            num_image_patches = len(image_num_patches) if hasattr(image_num_patches, "__len__") and not isinstance(image_num_patches, (int, float)) else 1
            for i in range(num_image_patches):
        ...
        if len(videos) != 0 and video_pixel_values is not None:
            ...
            for i in range(len(videos)):
                ...
                start_index = video_num_patches_indices[current_patch_index - 1] if current_patch_index > 0 else 0
    ...
    def process_messages(
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                num_patches_this = (
                    image_pixel_patch_list[num_image_tokens]
                    if image_pixel_patch_list is not None
                    else 1
                )
                if hasattr(num_patches_this, "item"):
                    num_patches_this = num_patches_this.item()
                content = content.replace(
                    IMAGE_PLACEHOLDER,
                    f"<img>{'<IMG_CONTEXT>' * image_seqlen * num_patches_this}</img>",
                    1,
                )
                num_image_tokens += 1

            while VIDEO_PLACEHOLDER in content:
                if video_patch_indices is not None and video_num_patches is not None:
                    ...
                    video_replaced_prompt = f"<img>{'<IMG_CONTEXT>' * image_seqlen}</img>"
...
class LlavaPlugin(BasePlugin):
    ...
    def _tokenizer_image_token(self, text: str, tokenizer: "PreTrainedTokenizer", image_token_index: int = -200) -> list[int]:
        """Convert <image> tokens to special index like GeoChat's tokenizer_image_token."""
        # Try to use GeoChat's native tokenizer_image_token if available
        try:
            from geochat.mm_utils import tokenizer_image_token
            return tokenizer_image_token(text, tokenizer, image_token_index, return_tensors=None)
        except ImportError:
            # Fallback implementation
            prompt_chunks = [tokenizer(chunk).input_ids for chunk in text.split('<image>')]
    ...
    def process_messages(
        ...
        messages = deepcopy(messages)
        if self.expand_mm_tokens:
            # For GeoChat models, use correct visual token count
            if hasattr(processor, 'image_processor') and (
                'CLIPImageProcessor' in processor.image_processor.__class__.__name__
                or 'RSLLaVAProcessorWrapper' in processor.image_processor.__class__.__name__
                or 'InternVL3ProcessorWrapper' in processor.__class__.__name__
            ):
                # GeoChat / RS-LLaVA / InternVL3: use processor.image_seq_length
                image_seqlen = processor.image_seq_length
                ...
                if "pixel_values" in mm_inputs:
                    ...
                    if processor.vision_feature_select_strategy == "default":
                        image_seqlen -= 1
                    # Add safety check to prevent token explosion (especially for GeoChat)
                    if image_seqlen > 1000:  # Reasonable maximum
                        image_seqlen = 1  # Fall back to single token for safety
                else:
                    image_seqlen = 1
        else:
            image_seqlen = 1
    ...
    def process_token_ids(
        ...
        """Process token IDs to replace <image> tokens with IMAGE_TOKEN_INDEX (-200)."""
        self._validate_input(processor, images, videos, audios)
        
        if not images:
            return input_ids, labels
            
        # Convert input_ids to text to check for <image> tokens
        text = tokenizer.decode(input_ids, skip_special_tokens=False)
        
        # If text contains <image>, use our tokenizer_image_token method
        if '<image>' in text:
            # Replace <image> with IMAGE_TOKEN_INDEX (-200)
            new_input_ids = self._tokenizer_image_token(text, tokenizer, image_token_index=-200)
            
            # Adjust labels if needed
            if labels is not None:
                # Simple approach: extend or truncate labels to match new length
                if len(new_input_ids) > len(input_ids):
                    # Pad labels with IGNORE_INDEX
                    labels = labels + [IGNORE_INDEX] * (len(new_input_ids) - len(input_ids))
                elif len(new_input_ids) < len(input_ids):
                    # Truncate labels
                    labels = labels[:len(new_input_ids)]
    ...
    def get_mm_inputs(
        ...
        """Get multimodal inputs. For GeoChat, we still process images but trainer will rename pixel_values to images."""
        self._validate_input(processor, images, videos, audios)
        
        # For GeoChat models, still process images normally - trainer will handle the parameter name conversion
        if hasattr(processor, 'image_processor') and 'CLIPImageProcessor' in processor.image_processor.__class__.__name__:
            pass  # GeoChat model detected
        
        # Use standard processing for all models - trainer will handle GeoChat-specific parameter conversion
        return self._get_mm_inputs(images, videos, audios, processor)

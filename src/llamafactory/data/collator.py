...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        batch_question_ids = []  # 添加question_id批次列表
        
        # 🔑 检查是否有input_ids_aug字段（用于TTL训练）
        has_input_ids_aug = any("input_ids_aug" in f for f in features)
        batch_input_ids_aug = [] if has_input_ids_aug else None
        
        for feature in features:
            images = feature.pop("images", None) or []
            videos = feature.pop("videos", None) or []
            audios = feature.pop("audios", None) or []
            question_id = feature.pop("question_id", None)  # 提取question_id字段，和images保持一致
            ...
            batch_question_ids.append(question_id)  # 保存question_id到批次列表
            
            # 🔑 保存input_ids_aug用于后续padding
            if has_input_ids_aug:
                batch_input_ids_aug.append(feature.get("input_ids_aug", feature["input_ids"]))
        ...
        # Avoid BatchEncoding.to() crash on non-tensor fields like question_id
        if "BatchEncoding" in str(type(features)) or hasattr(features, "data"):
            features = dict(features)

        # 🔑 TTL训练：手动padding input_ids_aug
        if batch_input_ids_aug is not None:
            # 使用与input_ids相同的padding参数
            input_ids_aug_features = [{"input_ids": ids_aug} for ids_aug in batch_input_ids_aug]
            # 使用父类的padding逻辑
            from transformers import DataCollatorForSeq2Seq
            temp_collator = DataCollatorForSeq2Seq(
            ...
            padded_aug = temp_collator(input_ids_aug_features)
            features["input_ids_aug"] = padded_aug["input_ids"]
        ...
        # 将question_id添加到features中，确保传递给下游函数
        if batch_question_ids and any(qid is not None for qid in batch_question_ids):
            features["question_id"] = batch_question_ids
        # 不再传递 ground_truth 字段
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        for key in ("chosen", "rejected"):
            for feature in features:
                ...
                # 如果存在question_id字段，也要保留
                if "question_id" in feature:
                    target_feature["question_id"] = feature["question_id"]
...
class KTODataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        for feature in features:
            ...
            if "question_id" in feature:
                target_feature["question_id"] = feature["question_id"]
                kl_feature["question_id"] = feature["question_id"]
...
@dataclass
class SubSamplesDataCollator(MultiModalDataCollatorForSeq2Seq):
    r"""专门为SubSamples格式设计的数据collator"""

    def __call__(self, features: list[dict[str, Any]]) -> list[list[dict]]:
        """
        处理SubSamples格式的数据
        
        输入features: [
            {
                "input_ids": [[ids1, ids2, ...], [ids3, ids4, ...]],  # 每个元素是一个parent的所有子问题
                "attention_mask": [[mask1, mask2, ...], [mask3, mask4, ...]],
                "labels": [...],
                "images": [...], 
                "question_id": [...],
                "input_ids_aug": [...]
        ...
        输出: [
            [dict1, dict2, ...],  # parent1的K个子问题
            [dict1, dict2, ...],  # parent2的K个子问题
            ...
        ]
        """
        batch_data = []
        ...
        for feature in features:
            # 重新组织每个parent的子问题数据
            parent_input_ids = feature.get("input_ids", [])
            parent_attention_masks = feature.get("attention_mask", [])
            parent_labels = feature.get("labels", [])
            parent_images = feature.get("images", [])
            parent_question_ids = feature.get("question_id", [])
            parent_input_ids_aug = feature.get("input_ids_aug", [])
            
            # 将parent级别的数据转换为子问题列表
            sub_questions_list = []
            if parent_input_ids:  # 确保有数据
                for i in range(len(parent_input_ids)):
                    sub_question = {
                        "input_ids": parent_input_ids[i],
                        "attention_mask": parent_attention_masks[i] if i < len(parent_attention_masks) else [1] * len(parent_input_ids[i]),
                        "labels": parent_labels[i] if i < len(parent_labels) else parent_input_ids[i],
                        "images": parent_images[i] if i < len(parent_images) else [],
                        "question_id": parent_question_ids[i] if i < len(parent_question_ids) else "",
                        "input_ids_aug": parent_input_ids_aug[i] if i < len(parent_input_ids_aug) else parent_input_ids[i],
                    }
                    sub_questions_list.append(sub_question)
                
                batch_data.append(sub_questions_list)

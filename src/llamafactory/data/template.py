...
# llava template without system message
register_template(
    name="llava_no_system",
    format_user=StringFormatter(slots=["USER: {{content}} ASSISTANT:"]),
    default_system="",  # 无系统消息
    mm_plugin=get_mm_plugin(name="llava", image_token="<image>"),
)
...
# InternVL3: Qwen-style chat, <image> placeholder, align with LlamaFactory intern_vl (<img><IMG_CONTEXT>...</img>)
register_template(
    name="internvl3",
    format_user=StringFormatter(slots=["<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n"]),
    format_assistant=StringFormatter(slots=["{{content}}<|im_end|>\n"]),

...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments, GaloreArguments, BAdamArgument):
    ...
    aug_entropy_weight: float = field(
        default=0.3,
        metadata={"help": "Weight coefficient for aug_entropy_loss in online_ttl total_loss: total_loss = -kl_prcp_loss + aug_entropy_loss * aug_entropy_weight."}
    )

    use_sft_loss: bool = field(
        default=False,
        metadata={"help": "Whether to add a supervised (CE) loss on ground-truth labels alongside the online_ttl loss."}
    )

    sft_loss_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for the SFT CE loss when use_sft_loss=True: total_loss = ttl_loss + sft_loss_weight * sft_loss."}
    )

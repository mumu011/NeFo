...
class DataArguments:
    ...
    enable_negation_filtering: bool = field(
        default=False,
        metadata={
            "help": (
                "Enable negation word filtering. When enabled, negation words in captions "
                "will be replaced with <Neg_Mask> tokens."
            )
        },
    )
    negation_words: List[str] = field(
        default_factory=lambda: ["no", "not", "without", "non-"],
        metadata={
            "help": (
                "List of negation words to filter from captions. "
                "Default: ['no', 'not', 'without', 'non-']"
            )
        },
    )

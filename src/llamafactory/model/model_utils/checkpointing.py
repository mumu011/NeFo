...
def get_custom_gradient_checkpointing_func(gradient_checkpointing_func: Callable) -> Callable:
    ...
    def custom_gradient_checkpointing_func(func: Callable, *args: Union["torch.Tensor", Any], **kwargs):
        """Wrap gradient checkpointing to only force requires_grad on floating tensors
        when the underlying module has trainable params. Robust to functools.partial.
        """

        # Try to recover the bound module from func; handle functools.partial as well.
        module = getattr(func, "__self__", None)
        if module is None and isinstance(func, partial):
            inner = func.func
            module = getattr(inner, "__self__", None)

        # Only enable grads for floating inputs if module is known and trainable.
        try:
            ...
            # Be permissive: if anything goes wrong, fall back to the original behavior.
            pass
